...
| Camera | **Run on hardware; `0x68` answers nothing, and the bus and the clock are both ruled out.** Decoded register-for-register in [`docs/sl6806_re_notes.md`](docs/sl6806_re_notes.md) §7h. `examples/CameraDemo` proves the bus against the FM tuner and delivers a measured **2.8 MHz** MCLK — the vendor programs channel 6 with 2800 — for 30 ms before it asks anything, under eight combinations. Nothing answers. The leading explanation is now that **this unit has no camera fitted**: the firmware supports one and the pads are real, but cheap players ship one image across several builds. Check for a lens before reading any further into it. |
