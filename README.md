...
| Camera — the pixel path | **Mapped and enabled; the sensor still will not start.** The DVP front end at `0x400E1000` is decoded in [`cores/sl6806/sl6806_dvp.h`](cores/sl6806/sl6806_dvp.h) (§7n) — geometry, crop, scaler, and the DMA destination register found by mapping the live block. It can be woken: mask ROM module id 46 for its registers, then `0x400E0000` bit 6 for its logic — which is §15's "dead" register, alive once something gates it open. On hardware the block accepts and reads back a full configuration, and the sensor's own output pins still sit high-impedance against a pull-up. Rail, enables, bus, pads, PLL, both clock families and all 64 camera-clock settings have been applied. That is a complete negative, not a missing step; §7n says what would settle it. **Written, not yet run:** [`cores/sl6806/sl6806_dvp.c`](cores/sl6806/sl6806_dvp.c) now drives the `+0x30..0x3C` DMA cluster that DvpProbe only mapped — address, then length, then a start trigger, this driver's own convention since no vendor code ever writes these four registers — and `examples/CameraDemo` fires it right after `dvpBringUp()`, independently of the sensor. 31 host tests hold it to its own contract; nothing here says whether a byte moves, only whether the descriptor registers hold what they are given (`sl6806_dvp_capture_writable()`) and, if they do, whether the destination buffer changes at all (the same before/after-fill method `AudioWall` used on the audio DMA). |
