...
| Camera | **Run on hardware; bus, clock and reset all ruled out, `0x68` still silent.** Decoded register-for-register in [`docs/sl6806_re_notes.md`](docs/sl6806_re_notes.md) §7h. `examples/CameraDemo` proves the bus against the FM tuner, delivers a measured **2.8 MHz** MCLK (the vendor programs channel 6 with 2800) for 30 ms before asking anything, and confirms RESET drives high at the vendor's drive strength — under eight combinations, nothing answers. Measuring the nets themselves puts the remaining suspicion on **sensor power**: PWDN carries an external pull-up, so the board does have camera circuitry, but every rail is behind a regulator nothing here drives and the vendor's own power-up has no such call in it. |
