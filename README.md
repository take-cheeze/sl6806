...
3. **Run `examples/RegFileProbe`, then `examples/CameraDemo`, and report what
   they print.** ~~Where the indexed register file lives~~ — found, in §7m, and
   it is a chip at I2C `0x30` behind a mailbox rather than a base address,
   which is why five static searches for a base could not have worked. That
   unlocks the camera's rail and 140 call sites besides, and *none of it has
   been run*. The probe reads the file and decodes its five LDO channels into
   millivolts without writing anything: if those come out at recognisable
   supply voltages, the decode is right, and that is a two-minute test no
   meter is needed for. `CameraDemo` then does the writes and verifies each
   one by reading it back.
4. **Break the `0x400E0000` wall, and two peripherals come with it.** This is
   the register the application enables most peripherals through, and §15
   recorded it as dead from a payload. That negative is now worth re-testing,
   because everything it lacked exists: the camera front end is id 6, there is
   a *second* register at `0x400E0008` that does a reset pulse rather than an
   enable (§7n), and there is finally a downstream block — the DVP's size
   registers at `0x400E1004` — that reads back what was written to it if it is
   alive and does not if it is not. The recipe is five steps and it is in
   [`sl6806_dvp.h`](cores/sl6806/sl6806_dvp.h). The backlight's counter is
   stuck behind the same register, so this is two results for one experiment.
...
examples/         Hello, Blink, GfxDemo, TouchDemo, CameraDemo, FmDemo,
                  ...
                  BacklightHunt, MmioProbe, RomProbe, CallbackProbe
