...
| Starting the vendor's firmware from a payload | **Written 2026-08-14, not yet run.** `examples/FirmBoot` copies the FIRM segment out of the flash image already on the device into SRAM and enters it — no flash write, and a power cycle undoes it, which is the whole difference from `MODE=firmware`. The load address is not in the header: it is the entry less the 256-entry vector table (§7m), and [`sl6806_firm.c`](cores/sl6806/sl6806_firm.c) derives it and then checks it against the image's own vector table before jumping, because getting it wrong is a hard fault with USB already gone. 15 host tests, all refusals bar one. It exists to ask what bootloader mode withholds: the mask ROM leaves both the SD host and the serial port switched off there, and one of the two still will not run a command after a payload wakes it (§28). |
